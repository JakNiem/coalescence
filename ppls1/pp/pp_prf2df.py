...
def prf2df(folderSims,flgExport=False,outFile='filenameForExport',configFile='config*.xml',quietMode=False):
    ...
    flist_chemPot  = sorted(glob.glob(folderSims+'/ChemPotSampling_TS*'))
    ...
    if flgMtdReadPrf == 2:
        ...
        for fp in fplists:
            ...
            dfTC = pd.read_csv(fp, delim_whitespace=True)
            timestep = int(fp.rpartition('TS')[-1].split('.')[0])
    ...
    if flgMtdReadPrf == 3: # NOT WORKING YET
        ...
        dfTemp1 = pd.read_csv(flist_scal_all[0], delim_whitespace=True)
        
        timesteps = list()
        
        for fp in flist_scal_all:
            timestep = int(fp.rpartition('TS')[-1].split('.')[0])
            if timestep not in timesteps:
                timesteps.append(timestep)
            
        components = np.arange(int((len(dfTemp1.columns)-1)/13))
        pos = dfTemp1.loc[:,'pos']
        
        li2= []
        for (_,val) in fplists.items():
            li1= []
            for fp in val:
                if not quietMode: print(fp)
                direction = fp.rpartition('quant_')[-1].split('_reg')[0]
                ...
                for comp in components:
                    ...
                    dfTemp.columns = dfTemp.columns.str.replace('\['+str(comp)+'\]','')+'_'+direction
                    dfTemp.loc[:,'cid'] = comp
                    dfTemp.loc[:,'timestep'] = timestep
                    li1.append(dfTemp)
            
            li2.append(pd.concat(li1,axis=0,ignore_index=True))
        
        dfProfScalVect = pd.concat(li2,axis=1,ignore_index=False)
        dfProfScalVect = dfProfScalVect.assign(pos=np.tile(pos,int(dfProfScalVect.shape[0]/len(pos))))
        
        li1= []
        for fp in flist_chemPot:
            if not quietMode: print(fp)
            timestep = int(fp.rpartition('TS')[-1].split('.')[0])
            for comp in components:
                dfTemp = pd.read_csv(fp, delim_whitespace=True)
                dfTemp.loc[:,'cid'] = comp
                dfTemp.loc[:,'timestep'] = timestep
                li1.append(dfTemp)
                
        dfProfChemPot = pd.concat(li1,axis=0,ignore_index=True)
        
        ## Concat all df and delete duplicate columns
        dfProfScalVect = dfProfScalVect.loc[:,~dfProfScalVect.columns.duplicated()]
        
        if dfProfChemPot['timestep'].equals(dfProfScalVect['timestep']): dfProfChemPot.drop('timestep', axis=1, inplace=True)
        if dfProfChemPot['cid'].equals(dfProfScalVect['cid']): dfProfChemPot.drop('cid', axis=1 ,inplace=True)
        if dfProfChemPot['pos'].equals(dfProfScalVect['pos']): dfProfChemPot.drop('pos', axis=1 ,inplace=True)
        
        dfProfTemp = dfProfChemPot.join(dfProfScalVect)
        
        dfProfTemp = dfProfTemp.loc[:,~dfProfTemp.columns.duplicated()]
    ...
    ## Export to JSON in 'columns' format
    if flgExport == True:
        df2json(dfProfTemp, metaData, outFile)
...
if __name__ == '__main__':
    ...
    if flgMtdMain == 1:
        ...
        simPath = '/home/pcfsuser/Simon/projects/005_Interface_resistivity/simulations/1clj/statEvap/T082_v01/run02'
        flgExport = False
